...
import { notification, Modal, Checkbox, Row, Col } from 'antd';
...
export function *fetchModuleDetail(action, { call, put, select }, namespace) {
  const { data } = yield call(() => request({ errormsg: '请求失败', ...action }, {}, { body: undefined, method: 'GET', Url: `${iface.moduleDetail}/${action.payload}` }));
...
export function *fetchModuleAdd(action, { call, put, select }, namespace) {
  ...
  notification.success({ message: '插入成功', description: '新增模块成功' });
}
...
export function *fetchModuleEdit(action, { call, put, select }, namespace) {
  ...
  notification.success({ message: '更新成功', description: '修改模块成功' });
}
...
export function updateModuleParentAll(state, action) {
  return update(state, {
    ...
    req: { fields: { parentId: { value: { $set: ['0'] } } } },
  });
}
...
export function *fetchModulesetButton(action, { call, put, select }, namespace) {
  ...
  notification.success({ message: '更新成功', description: '修改设置按钮成功' });
}
