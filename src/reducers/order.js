...
export function *fetchOrderCenConPagingAgency(action, { call, put, select }, namespace) {
  ...
  const { data, headers } = yield call(() => request({ errormsg: '表格数据请求失败', ...action }, {}, { body: req, method: 'POST', Url: iface.orderCenConPagingAgency }));
...
export function *fetchOrderCenConPagingSupplier(action, { call, put, select }, namespace) {
  ...
  const { data, headers } = yield call(() => request({ errormsg: '表格数据请求失败', ...action }, {}, { body: req, method: 'POST', Url: iface.orderCenConPagingSupplier }));
