...
export function *fetchPermissionAdd(action, { call, put, select }, namespace) {
  ...
  notification.success({ message: '插入成功', description: '新增权限成功' });
}
...
export function *fetchPermissionEdit(action, { call, put, select }, namespace) {
  ...
  notification.success({ message: '更新成功', description: '修改权限成功' });
}

/* 查看权限 */
export function *fetchPermissionDetail(action, { call, put, select }, namespace) {
  const { data } = yield call(() => request({ errormsg: '请求失败', ...action }, {}, { body: undefined, method: 'GET', Url: `${iface.permissionDetail}/${action.payload}` }));
