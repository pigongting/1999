...
class FormTablePage extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextres.rows !== res.rows) {
      ...
      this.setState(update(this.state, { clickedRows: { $set: [] } }));
    }

    // 外部点击行
    if (nextres.clickedRows !== res.clickedRows) {
      this.setState(update(this.state, { clickedRows: { $push: nextres.clickedRows } }));
    }
  }
  ...
  /* 点击行 */
  operateClickedRows(key) {
    this.setState(update(this.state, { clickedRows: { $push: [key] } }));
  }
  ...
  render() {
    const { nowColumns, tableSize, setColumnModalVisible, selectedRows, clickedRows } = this.state;
        ...
        <Layout className="tablePage">
          ...
          <Content className="tableContent" id="formScrollContent">
            ...
            <Table
              ...
              onRowClick={params => this.operateClickedRows(params.key)}
            />
